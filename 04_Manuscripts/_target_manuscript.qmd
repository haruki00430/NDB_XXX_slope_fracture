...
Hip fracture imposes a major burden on older adults in Japan. We conducted an ecological study of 47 prefectures to examine whether habitable-area-weighted terrain slope was associated with prefecture-level hip fracture surgery rates窶蚤dministrative proxies for fracture-related surgical burden窶蚤fter adjusting for population composition and walking indicators. Using the 10th National Database Open Data (Reiwa 5 claims, April 2023 through March 2024), we derived annual surgery rates per 100,000 population for hip, humerus, forearm, and total fractures; terrain slope from national 10-m elevation models; proportion aged 竕･65 years and population density from the 2020 census; and proportion reporting fast habitual walking from Reiwa 4 Specific Health Checkup data. Multivariable linear regression was used to test whether slope was independently associated with each surgery-rate outcome. Mean slope was 8.57 degrees (range 1.81 [Chiba] to 15.43 [Kochi]). Steeper slope was associated with higher hip fracture surgery rates after adjustment for proportion aged 竕･65 years, fast-walking proportion, and population density (approximately 3.5 additional hip surgeries per 100,000 per one-degree increase). Associations were not observed for humerus or forearm surgery rates. Steeper prefectures may experience greater hip-specific surgical burden, although ecological data cannot establish individual-level fracture incidence.
...
The proportion of individuals who reported walking fast (proportion with fast habitual walking, %) was derived from the NDB Specific Health Checkup (Tokutei Kenko Shinsa) questionnaire administered as part of the national annual health screening program. In the 10th NDB Open Data release, prefecture-level Specific Health Checkup tabulations correspond to **Reiwa 4 fiscal year** (April 2022 through March 2023), which does not coincide with the Reiwa 5 claims window used for fracture outcomes. Participants aged 40窶・4 years responded to a question on habitual walking speed, and we calculated the prefecture-level proportion of respondents who reported "fast" walking.

## 5. Additional Covariates

Proportion aged 竕･65 years (%) was obtained from the 2020 National Census. Population density (persons per kmﾂｲ) was calculated as total population divided by prefecture area from the same source.

## 6. Statistical Analysis

The primary analysis tested whether habitable-area-weighted terrain slope was linearly associated with hip fracture surgery rate after adjustment for proportion aged 竕･65 years, proportion with fast habitual walking, and population density. Secondary analyses applied the same regression framework to total, humerus, and forearm surgery rates. Exploratory regressions with proportion aged 竕･65 years or fast-walking proportion as outcomes were conducted to describe how contextual covariates co-varied with terrain slope at prefecture resolution (not interpreted as formal mediation).

We summarized prefecture-level distributions using mean, SD, median, 25th and 75th percentiles, minimum, and maximum. Pearson correlation coefficients were computed between terrain slope and each outcome and covariate variable. We fit two regression models for each surgery-rate outcome:

- **Model 1 (unadjusted)**: Simple linear regression of fracture surgery rate on terrain slope only.
- **Model 2 (multivariable)**: Multiple linear regression of fracture surgery rate on terrain slope, proportion aged 竕･65 years, proportion with fast habitual walking, and population density.
In the primary interpretation, proportion aged 竕･65 years and fast-walking proportion were treated as confounder-adjustment variables rather than formal mediators.

Model performance was assessed using the coefficient of determination (Rﾂｲ), adjusted Rﾂｲ, and the F-statistic. Regression coefficients (ﾎｲ) and 95% confidence intervals (CIs) are reported. Statistical significance was defined as *p* < 0.05 (two-sided). All analyses were performed using Python (version 3.14.2) with `statsmodels` (version 0.14.6). Linear regression models were estimated by ordinary least squares (OLS) using the `statsmodels` formula interface (`statsmodels.formula.api.ols`).

For the primary hip-fracture outcome (Model 2), we assessed linear model assumptions graphically using residuals-versus-fitted and normal quantile窶賭uantile (Q窶轍) plots of OLS residuals, and we report the Shapiro窶展ilk test on residuals for descriptive completeness (small-*N* tests are interpreted cautiously). Because prefecture-level ecological analyses can exhibit heteroskedasticity and leverage from skewed covariates (notably population density), we conducted sensitivity analyses: (1) heteroskedasticity-robust standard errors (HC3) for two-sided *p* values and CIs, and (2) a nonparametric bootstrap with *B* = 5,000 resamples of prefectures (with replacement) to obtain a percentile 95% CI for the terrain slope coefficient.
...
Across the 47 Japanese prefectures, the mean habitable-area-weighted terrain slope was 8.57 ﾂｱ 3.16 degrees (median 8.36; interquartile range [IQR] 6.56窶・0.95; range 1.81 [Chiba]窶・5.43 [Kochi]) (Table 1). The mean hip fracture surgery rate was 254.0 ﾂｱ 37.6 per 100,000 population per year (median 253.9; IQR 228.6窶・86.3; range 167.6窶・29.6), and the mean total fracture surgery rate was 352.8 ﾂｱ 54.9 per 100,000 (median 356.1; IQR 306.4窶・97.1). The mean proportion aged 竕･65 years was 30.7 ﾂｱ 3.1% (median 31.3%; IQR 29.3窶・2.9%), and the mean proportion with fast habitual walking was 48.6 ﾂｱ 4.3% (median 49.2%; IQR 46.2窶・1.2%). Population density was right-skewed (mean 657 persons/kmﾂｲ versus median 265; IQR 173窶・69).

## 2. Correlation Analysis

Bivariate correlations are summarized in Table 2 and illustrated for the primary exposure窶登utcome pair in Figure 1. Terrain slope correlated positively with hip fracture surgery rate (r = 0.475, *p* < 0.01) and with total fracture surgery rate (r = 0.394, *p* < 0.01). Slope correlated positively with proportion aged 竕･65 years (r = 0.505, *p* < 0.01) and showed a weak negative correlation with fast-walking proportion (r = 竏・.264, *p* = 0.073). Correlations with humerus and forearm surgery rates were weak and non-significant (r = 0.138 and 0.177, respectively).

## 3. Primary Regression Analysis: Hip Fracture Surgery Rate

In the unadjusted model (Model 1), terrain slope was significantly and positively associated with hip fracture surgery rate (ﾎｲ = 5.65; 95% CI, 2.50窶・.79; Rﾂｲ = 0.225; *p* = 0.001). After adjustment for proportion aged 竕･65 years, fast-walking proportion, and population density (Model 2), the slope coefficient was 3.49 (95% CI, 0.11窶・.86; Rﾂｲ = 0.385; *p* = 0.043) using conventional (model-based) standard errors (Table 3). Proportion aged 竕･65 years was also associated with hip fracture surgery rate (ﾎｲ = 5.53; 95% CI, 1.32窶・.74; *p* = 0.011). Fast-walking proportion was not significant under conventional standard errors (ﾎｲ = 2.02; 95% CI, 竏・.34窶・.37; *p* = 0.091). Population density was not significantly associated with hip fracture surgery rate (ﾎｲ = 竏・.001; *p* = 0.818).

## 4. Sensitivity Analyses

Under heteroskedasticity-robust (HC3) standard errors and nonparametric bootstrap resampling of prefectures (*B* = 5,000), the adjusted slope coefficient for hip fracture surgery rate was borderline (HC3 *p* = 0.063; bootstrap 95% CI, 竏・.03 to 7.09; Table 3 footnote). Indirect standardization sensitivity analyses using alternative imputation values for suppressed age-stratified surgery cells yielded positive slope coefficients in all three prespecified scenarios (Table 5).

## 5. Regression Analysis: Total, Humerus, and Forearm Surgery Rates

In the unadjusted model, slope was significantly associated with total fracture surgery rate (ﾎｲ = 6.85; 95% CI, 2.06窶・1.64; Rﾂｲ = 0.156; *p* = 0.006). In the multivariable model, however, slope did not reach statistical significance for total fracture surgery rate (ﾎｲ = 4.47; *p* = 0.099), although proportion aged 竕･65 years remained a significant predictor (ﾎｲ = 7.19; *p* = 0.035; Rﾂｲ = 0.276). Neither humerus fracture surgery rate (Model 1: *p* = 0.355; Model 2: Rﾂｲ = 0.107) nor forearm fracture surgery rate (Model 1: *p* = 0.233; Model 2: Rﾂｲ = 0.062) was significantly associated with terrain slope in either model.
...
Proportion aged 竕･65 years was the strongest predictor of hip fracture surgery rate in the multivariable model (ﾎｲ = 5.53, *p* = 0.011). Prefectures with steeper terrain also tended to have higher aging rates (r = 0.505), reflecting the well-documented trend of rural aging in Japan, where mountainous prefectures experience greater outmigration of younger populations.[35, 36, 37] Older adults in these high-slope prefectures thus face a double burden: the physical challenge of navigating harsh terrain on a daily basis (a topographical contextual challenge) and the social isolation and lack of medical access associated with rural depopulation (social determinants of health).[38, 39]

The partial association of slope with hip fracture surgery rates after adjustment for proportion aged 竕･65 years suggests that terrain may constitute an environmental correlate over and above demographic composition, subject to the inferential sensitivity noted above. This finding has practical implications: even within the context of Japan's nationwide aging, geographic topography may represent an additional contextual correlate of surgical burden among older residents. Terrain slope may not be merely a proxy for aging and may represent an additional environmental correlate that could contribute to functional decline among rural-dwelling older adults.
...
[4] Nakatoh S, Fujimori K, Ishii S, Iki M. Trends in the incidence of hip fractures and osteoporosis treatment in Japan (FY2012窶擢Y2023). Arch Osteoporos. 2025; 21(1):16.
...
[20] Dahl C, Madsen C, Omsland TK, Sﾃｸgaard AJ, Tell GS, Holvik K, Meyer HE. Contribution of elevation and residential proximity to the coast in explaining geographic variations in hip fracture incidence. A Norwegian Epidemiologic Osteoporosis Studies (NOREPOS) study. Osteoporos Int. 2021;32(5):1001-1006.
...
[29] Hak L, Houdijk H, Steenbrink F, Mert A, van der Wurff P, Beek PJ, van Dieﾃｫn JH. Stepping strategies for regulating gait adaptability and stability. J Biomech. 2013;46(5):905-911.
...
[33] Dahl C, Madsen C, Omsland TK, et al. The association of cold ambient temperature with fracture risk and mortality: national data from Norway窶蚤 Norwegian Epidemiologic Osteoporosis Studies (NOREPOS) study. J Bone Miner Res. 2022;37(8):1527-1536.
...
| **Covariate variables** | | | | | | | | | |
| Proportion aged 竕･65 years | % | 47 | 30.74 | 3.10 | 31.26 | 29.28 | 32.88 | 22.61 | 36.45 |
| Proportion with fast habitual walking | % | 47 | 48.62 | 4.34 | 49.23 | 46.23 | 51.22 | 29.79 | 57.30 |
| Population density | persons/kmﾂｲ | 47 | 657.00 | 1,223.13 | 265.44 | 173.20 | 468.68 | 62.63 | 6,402.64 |

*Note.* Q1 and Q3 denote 25th and 75th percentiles (prefecture-level observations). Exploratory Shapiro窶展ilk tests rejected normality for fast-walking proportion and population density (both *p* < 0.001); other listed variables were not rejected at ﾎｱ = 0.05 (proportion aged 竕･65 years *p* = 0.090). Marginal normality is not required for ordinary least squares; median and quartiles are presented to describe skewed distributions alongside mean and SD.

---

## Table 2. Pearson Correlation Matrix of Study Variables (N = 47)

| Variable | Hip Fracture | Total Fracture | Humerus Fracture | Forearm Fracture | Terrain Slope | Proportion 竕･65 y | Fast Walking Proportion | Pop Density |
|:---|:---:|---:|---:|---:|---:|---:|---:|---:|
| Hip Fracture | 1.000 |  |  |  |  |  |  |  |
...
| Proportion 竕･65 y | 0.533*** | 0.430** | 0.152 | 0.156 | 0.505*** | 1.000 |  |  |
| Fast Walking Proportion | 竏・.044 | 0.011 | 0.177 | 0.060 | 竏・.264 | 竏・.406** | 1.000 |  |
| Pop Density | 竏・.336* | 竏・.238 | 0.006 | 竏・.032 | 竏・.345* | 竏・.628*** | 0.381** | 1.000 |
...
| :--- | ---: | ---: | ---: | ---: |
| **Variable** | **ﾎｲ (95% CI)** | ***p*** | **ﾎｲ (95% CI)** | ***p*** |
| Terrain slope (per 1 degree) | **5.65 (2.50, 8.79)** | **0.001** | **3.49 (0.11, 6.86)** | **0.043** |
| Proportion aged 竕･65 y (per 1%) | 窶・| 窶・| **5.53 (1.32, 9.74)** | **0.011** |
| Fast-walking proportion (per 1%) | 窶・| 窶・| 2.02 (竏・.34, 4.37) | 0.091 |
| Population density (per 1 person/kmﾂｲ) | 窶・| 窶・| 竏・.001 (竏・.01, 0.01) | 0.818 |
| **Rﾂｲ** | 0.225 | | 0.385 | |
| **Adjusted Rﾂｲ** | 0.208 | | 0.326 | |
| **F-statistic (*p*)** | 13.23 (0.001) | | 6.56 (0.0003) | |

Model 1: simple linear regression of hip fracture rate on terrain slope alone.
Model 2: multivariable linear regression adjusting for proportion aged 竕･65 years, fast-walking proportion, and population density.
ﾎｲ, unstandardized regression coefficient; CI, confidence interval.
**Bold** indicates *p* < 0.05 (conventional model-based standard errors).

*Sensitivity (Model 2, terrain slope coefficient):* heteroskedasticity-robust (HC3) ﾎｲ = 3.49 (95% CI, 竏・.19窶・.17; *p* = 0.063); nonparametric bootstrap 95% CI (prefecture resampling, *B* = 5,000) = 竏・.03窶・.09.

---

## Table 4. Linear Regression Results for All Fracture Outcomes: Model 2 (Multivariable, N = 47)

| **Outcome** | **Slope ﾎｲ (95% CI)** | ***p*** | **Proportion 竕･65 y ﾎｲ (95% CI)** | ***p*** | **Rﾂｲ** | **Adj. Rﾂｲ** |
| :--- | ---: | ---: | ---: | ---: | ---: | ---: |
| Hip (femur) fracture | **3.49 (0.11, 6.86)** | **0.043** | **5.53 (1.32, 9.74)** | **0.011** | **0.385** | **0.326** |
| Total fracture | 4.47 (竏・.88, 9.82) | 0.099 | **7.19 (0.51, 13.86)** | **0.035** | 0.276 | 0.206 |
| Humerus fracture | 0.23 (竏・.50, 0.96) | 0.524 | 0.54 (竏・.38, 1.46) | 0.245 | 0.107 | 0.022 |
| Forearm fracture | 0.67 (竏・.95, 2.30) | 0.402 | 1.28 (竏・.77, 3.33) | 0.218 | 0.062 | 竏・.027 |

All models adjusted for proportion aged 竕･65 years, fast-walking proportion, and population density.
ﾎｲ, unstandardized regression coefficient; CI, confidence interval.
...
| **Imputation scenario for suppressed age-stratified surgery cells** | **Terrain slope ﾎｲ (95% CI)** per 1ﾂｰ | ***p*** |
| :--- | :--- | :--- |
| Suppressed cell imputed as 0 surgeries | **5.71 (0.54, 10.88)** | **0.031** |
| Suppressed cell imputed as 5 surgeries | **3.61 (0.30, 6.92)** | **0.033** |
| Suppressed cell imputed as 9 surgeries | **3.64 (0.28, 7.00)** | **0.035** |

Each row replaces the crude outcome in Table 3 Model 2 with the indirectly standardized hip (femur) surgery rate computed under the listed imputation rule (Methods). ﾎｲ, unstandardized regression coefficient; CI, confidence interval. **Bold** indicates *p* < 0.05 (conventional model-based standard errors). These estimates are for sensitivity analysis only.
