...
As an additional sensitivity analysis for age-structure confounding, we performed an indirect standardization workflow for the femur-surgery outcome. We used NDB age-stratified surgery tables and 2020 census 5-year age-group population counts from e-Stat. Because disclosure-control masking (`"-"`) appeared in age-stratified surgery cells, we prespecified pragmatic imputation scenarios (`"-" = 0`, `5`, or `9`). We repeated the SIR/ISR workflow under each scenario. For each scenario, we calculated national age-specific surgery rates, expected prefectural counts, SIR, and ISR (SIR × national crude rate). We then regressed ISR on terrain slope with and without covariate adjustment. We treated these analyses as sensitivity analyses only, not as primary inference, because secondary masking might remain.
...
Across the 47 Japanese prefectures, the mean habitable-area-weighted terrain slope was 8.57 ± 3.16 degrees (median 8.36; interquartile range [IQR] 6.56–10.95; range 1.81–15.43) (Table 1). The mean hip fracture surgery rate was 254.0 ± 37.6 per 100,000 population (median 253.9; IQR 228.6–286.3; range 167.6–329.6), and the mean total fracture surgery rate was 352.8 ± 54.9 per 100,000 (median 356.1; IQR 306.4–397.1). The mean aging rate was 30.7 ± 3.1% (median 31.3%; IQR 29.3–32.9%), and the mean proportion with fast walking speed was 48.6 ± 4.3% (median 49.2%; IQR 46.2–51.2%). Population density was right-skewed (mean 657 persons/km² versus median 265; IQR 173–469).
...
Residual diagnostics for Model 2 did not indicate gross nonlinearity in residuals versus fitted values. The Shapiro–Wilk test on OLS residuals was *W* = 0.957 (*p* = 0.079). Sensitivity analyses indicated inferential fragility for the primary slope coefficient at *N* = 47. Under heteroskedasticity-robust (HC3) inference, the 95% CI was −0.19 to 7.17 (*p* = 0.063). Under nonparametric bootstrap resampling of prefectures (*B* = 5,000), the 95% CI was −0.03 to 7.09. Both intervals included the null. We therefore interpreted the adjusted slope-hip fracture association as statistically borderline under alternative variance assumptions for ecological units.

In masking-imputation indirect-standardization sensitivity analyses, the slope coefficient was positive in all scenarios. In multivariable models (ISR ~ slope + aging rate + fast walking rate + population density), β was 5.71 (95% CI, 0.54–10.88; *p* = 0.031) for `"-" = 0`, 3.61 (95% CI, 0.30–6.92; *p* = 0.033) for `"-" = 5`, and 3.64 (95% CI, 0.28–7.00; *p* = 0.035) for `"-" = 9`. The direction was stable, whereas effect size varied across masking assumptions.
...
This ecological study found a positive adjusted association between prefectural terrain slope and hip fracture surgery rates (β = 3.49 per degree; conventional 95% CI, 0.11–6.86; *p* = 0.043). The model adjusted for aging rate, walking speed, and population density. HC3 and bootstrap intervals were compatible with no association at α = 0.05. Under the primary OLS parameterization, each additional degree of slope corresponded to about 3.5 additional hip fracture surgeries per 100,000 population. The adjusted signal was specific to hip fractures and was not observed for humerus or forearm fractures. Figures 3–5 show prefectural distributions of terrain slope, hip fracture rates, and their joint classification. Overall, these findings suggest that steeper habitable terrain may track hip-specific surgical burden beyond compositional aging and self-reported walking pace. This pattern is consistent with terrain-related lateral-fall mechanisms, but confirmation under more robust approaches is needed for small ecological *N*.

## 2. Biological Plausibility

The observed hip-specific pattern is biologically plausible and may be partly supported by biomechanical evidence. Hip fractures typically result from lateral (sideways) falls,[22, 23] which are a common fall mechanism on inclined surfaces. Real-world video-capture studies of falls in long-term care facilities have demonstrated that lateral landing configuration increases hip fracture risk by 5.5 to 6-fold compared to forward or backward falls, and direct impact to the greater trochanter elevates fracture risk by up to 30-fold.[23, 24] The impact forces generated during lateral falls typically reach 2,000 to 4,000 N, exceeding the physiological tolerance of osteoporotic femoral necks.[25] Finite element method (FEM) simulations have further shown that posterolateral and lateral impacts generate maximum tensile and compressive stresses at the superior and inferior aspects of the femoral neck, potentially increasing fracture vulnerability.[26]
...
The partial association of slope with hip fracture after adjustment for aging rate suggests that terrain may contribute risk beyond demographic composition. However, this interpretation remains sensitive to inference choices. The finding has practical implications because topography may represent an additional and potentially modifiable layer of fracture risk in an aging society. At the same time, residual confounding and ecological design constraints remain. Therefore, slope may still partly reflect demographic and healthcare-context differences across prefectures.

Given public-data masking in age-stratified surgery tables, we interpreted age-structure robustness primarily from two findings: consistency of adjusted ecological models and consistency of imputation-based sensitivity analyses. We also acknowledge that full indirect standardization without imputation was not feasible with the current open dataset.
...
However, the multivariable OLS coefficient for fast walking rate was **positive** (β = 2.02 per 1 percentage-point increase in fast walking). This result is difficult to interpret as a causal protective effect at prefecture resolution and differs from individual-level cohort findings. Inference was sensitive to standard error choice (conventional *p* = 0.091 versus HC3 *p* = 0.022). Possible explanations include ecological bias, use of a 40–74-year screening metric in regions where hip fractures concentrate among the oldest-old, and unmeasured healthcare and reporting factors. We therefore did not treat walking speed as a statistically supported mediator of the slope-hip fracture association. Future individual-level studies (e.g., measured gait speed and accelerometry) with formal causal mediation analysis are needed to test whether terrain slope influences hip fracture risk partly through chronic effects on habitual walking capacity.
...
Second, our outcome reflects surgery rates rather than true fracture incidence; although hip fractures are usually treated surgically, prefectural variation in treatment decisions, referral patterns, and timely access to operative care may influence observed rates independently of fracture occurrence. Third, the cross-sectional design precludes causal inference. Although reverse causation is unlikely for a fixed environmental exposure such as terrain slope, residual confounding by unmeasured variables cannot be excluded. Fourth, important unmeasured confounders—including individual-level socioeconomic status, healthcare access (e.g., prefectural orthopedic surgeon density, distance to emergency hospitals), nutritional status (calcium and vitamin D intake), bone mineral density (BMD), climate (snowfall/icing), and medication use (bisphosphonates, vitamin D supplementation)—may account for part of the observed association. Future studies incorporating NDB prescription data and BMD screening results at finer geographic resolution would strengthen causal inference.

Fifth, age-standardization sensitivity results relied on imputation of masked (`"-"`) cells in publicly released age-stratified surgery tables. Directionality was robust across `0/5/9` scenarios, but effect size depended on masking assumptions. Secondary masking rules also imply that some masked cells may not correspond strictly to the nominal `<10` range. Therefore, we interpreted these SIR/ISR analyses as robustness checks rather than definitive age-standardized estimates. Sixth, the walking speed variable was based on self-reported habitual walking speed in the Specific Health Checkup questionnaire for individuals aged 40–74 years. This variable may not represent the functional capacity of the oldest-old (≥75 years), who are at the highest fracture risk. The self-reported measure may also include non-differential measurement error, which could bias estimates toward the null. Seventh, the DEM-based slope calculation reflects topographic exposure at the prefecture level and does not capture micro-scale terrain variation (e.g., steep residential pathways and stairways) encountered in daily life. High-resolution spatial analyses at municipal or neighborhood levels may provide more granular insights into the terrain-fracture relationship.
...
Under primary ordinary least squares estimation, prefectural terrain slope was positively associated with hip fracture surgery rates after adjustment for aging rate, walking speed, and population density (β ≈ 3.5 additional surgeries per 100,000 per one-degree increase; conventional *p* = 0.043). However, heteroskedasticity-robust and bootstrap analyses yielded borderline support that was compatible with no association at α = 0.05. The adjusted signal was specific to hip fractures, and no significant relationship was observed for humerus or forearm fractures. These findings were consistent with biomechanical hypotheses that emphasize lateral falls and trochanteric loading on sloped terrain.
