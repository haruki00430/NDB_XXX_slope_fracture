...
As an additional sensitivity analysis for age-structure confounding, we implemented an indirect-standardization workflow for the femur-surgery outcome using NDB age-stratified surgery tables and 2020 census 5-year age-group population counts from e-Stat. Because disclosure-control masking (`"-"`) was present in age-stratified surgery cells, we prespecified pragmatic imputation scenarios for masked cells (`"-" = 0`, `5`, or `9`) and repeated the SIR/ISR workflow for each scenario. For each scenario, we calculated national age-specific surgery rates, expected prefectural counts, SIR, and ISR (SIR × national crude rate), then regressed ISR on terrain slope with and without covariate adjustment. We treated these analyses as sensitivity analyses only (not primary inference), given potential secondary masking.
