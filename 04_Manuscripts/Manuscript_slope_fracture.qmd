...
We repeated indirect-standardization sensitivity analyses under three prespecified rules for masked (`"-"`) cells in age-stratified surgery tables (Methods). We modeled indirectly standardized hip (femur) surgery rates (ISR; per 100,000) with terrain slope, aging rate, fast walking rate, and population density, as in Table 3 Model 2. The terrain slope coefficient was positive in all three scenarios (Table 5). Effect sizes varied with imputation assumptions, but the sign was stable.
...
## Table 5. Indirect Standardization Sensitivity Analysis: Multivariable Linear Regression of Indirectly Standardized Hip (Femur) Surgery Rate (ISR) on Terrain Slope (N = 47)

| **Imputation scenario for masked (`"-"`) age-stratified surgery cells** | **Terrain slope β (95% CI)** per 1° | ***p*** |
| :--- | :--- | :--- |
| `"-"` imputed as 0 | **5.71 (0.54, 10.88)** | **0.031** |
| `"-"` imputed as 5 | **3.61 (0.30, 6.92)** | **0.033** |
| `"-"` imputed as 9 | **3.64 (0.28, 7.00)** | **0.035** |

Each row replaces the crude outcome in Table 3 Model 2 with ISR from indirect standardization under the listed imputation rule (Methods). β, unstandardized regression coefficient; CI, confidence interval. **Bold** indicates *p* < 0.05 (conventional model-based standard errors). These estimates are for sensitivity analysis only.
