# -*- coding: utf-8 -*-
"""
メッシュ座標から都道府県を特定（修正版：より正確な緯度経度範囲）
"""
...
pref_ranges = {
    ...
    '埼玉県': {'lat': (35.7, 36.3), 'lon': (138.7, 140.0)},  # 修正：経度範囲を拡大
    '千葉県': {'lat': (34.9, 36.1), 'lon': (139.7, 140.9)},
    '東京都': {'lat': (20.0, 36.0), 'lon': (136.0, 154.0)},  # 小笠原諸島を含む
    '神奈川県': {'lat': (35.1, 35.6), 'lon': (138.9, 140.0)},  # 修正：経度範囲を拡大
    ...
    '奈良県': {'lat': (33.9, 34.8), 'lon': (135.6, 136.2)},  # 修正：緯度範囲を拡大
    '和歌山県': {'lat': (33.4, 34.4), 'lon': (135.1, 136.0)},
    '鳥取県': {'lat': (35.1, 35.6), 'lon': (133.2, 134.4)},  # 修正：緯度範囲を拡大
...
# 結果を保存
output_file = r"c:\Users\user\SharedWorkspace\projects\NDB_Research_Hub\projects\NDB_XXX_slope_fracture\03_Analysis\data\interim\mesh_prefecture_mapping_v2.csv"
result_df.to_csv(output_file, index=False, encoding='utf-8')

# レポート作成
report_lines = []
report_lines.append("=== G04-c メッシュの都道府県カバレッジ（修正版） ===\n")
...
# 特に確認：埼玉県、奈良県、神奈川県、鳥取県
target_prefs = ['埼玉県', '奈良県', '神奈川県', '鳥取県']
report_lines.append(f"\n=== 特記：ユーザー指摘の4都道府県 ===\n")
for pref in target_prefs:
    if pref in pref_mesh_counts:
        report_lines.append(f"✓ {pref}: {pref_mesh_counts[pref]}メッシュ（カバーされている）\n")
    else:
        report_lines.append(f"✗ {pref}: カバーされていない\n")

# レポートを保存
report_file = r"c:\Users\user\SharedWorkspace\projects\NDB_Research_Hub\projects\NDB_XXX_slope_fracture\03_Analysis\data\interim\prefecture_coverage_report_v3.txt"
