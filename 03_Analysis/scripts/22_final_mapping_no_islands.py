...
# 都道府県の緯度経度範囲（最終版：離島を除外）
# 北海道: 本島のみ（経度146.0°E未満）
# 東京都: 本州（35.5°N以北、138.9-140.0°E）と離島（35.0°N未満、136.5-154.0°E）
# 他の都道府県: 離島を含む範囲を設定
pref_ranges = {
    '北海道（本島）': {'lat': (41.0, 46.0), 'lon': (139.0, 146.0)},  # 経度146.0°E未満
    ...
    '東京都（離島）': {'lat': (20.0, 34.9), 'lon': (136.5, 154.0)},
...
for _, row in mesh_df.iterrows():
    ...
    for pref, ranges in pref_ranges.items():
        if check_overlap(s_lat, n_lat, w_lon, e_lon, ranges['lat'], ranges['lon']):
            # 北海道（本島）と東京都（本州）（離島）は統合
            if pref == '北海道（本島）':
                if '北海道' not in matched_prefs:
                    matched_prefs.append('北海道')
            elif pref.startswith('東京都'):
...
# 結果を保存
output_file = r"c:\Users\user\SharedWorkspace\projects\NDB_Research_Hub\projects\NDB_XXX_slope_fracture\03_Analysis\data\interim\mesh_prefecture_mapping_no_islands.csv"
result_df.to_csv(output_file, index=False, encoding='utf-8')

# レポート作成
report_lines = []
report_lines.append("=== G04-c メッシュの都道府県カバレッジ（離島除外版） ===\n")
report_lines.append(f"総メッシュ数: {len(result_df)}\n\n")

report_lines.append("=== 都道府県別メッシュ数（上位15） ===\n")
sorted_prefs = sorted(pref_mesh_counts.items(), key=lambda x: x[1], reverse=True)
for i, (pref, count) in enumerate(sorted_prefs[:15], 1):
    report_lines.append(f"{i:2d}. {pref}: {count}メッシュ\n")

# 特に確認：北海道、東京都
target_prefs = ['北海道', '東京都', '埼玉県', '奈良県', '神奈川県', '鳥取県']
report_lines.append(f"\n=== 特記：主要都道府県 ===\n")
for pref in target_prefs:
    if pref in pref_mesh_counts:
        report_lines.append(f"✓ {pref}: {pref_mesh_counts[pref]}メッシュ\n")
    else:
        report_lines.append(f"✗ {pref}: カバーされていない\n")

report_lines.append(f"\n=== サマリー ===\n")
report_lines.append(f"カバーされている都道府県数: {len(pref_mesh_counts)}\n")

# レポートを保存
report_file = r"c:\Users\user\SharedWorkspace\projects\NDB_Research_Hub\projects\NDB_XXX_slope_fracture\03_Analysis\data\interim\prefecture_coverage_report_no_islands.txt"
