# -*- coding: utf-8 -*-
"""
メッシュ座標から都道府県を特定（最終修正版：東京都の離島範囲を厳密に定義）
"""
...
pref_ranges = {
    ...
    # 東京都は本州部分と離島部分を厳密に分離
    # 本州: 35.5°N以北、138.9-140.0°E
    # 離島: 35.0°N未満、136.5-154.0°E（小笠原諸島・伊豆諸島）
    # 注：経度136.5°E以西は近畿地方のため除外
...
# 結果を保存
output_file = r"c:\Users\user\SharedWorkspace\projects\NDB_Research_Hub\projects\NDB_XXX_slope_fracture\03_Analysis\data\interim\mesh_prefecture_mapping_final_v2.csv"
result_df.to_csv(output_file, index=False, encoding='utf-8')

# レポート作成
report_lines = []
report_lines.append("=== G04-c メッシュの都道府県カバレッジ（最終修正版v2） ===\n")
...
# 特に確認：埼玉県、奈良県、神奈川県、鳥取県、東京都
target_prefs = ['東京都', '埼玉県', '奈良県', '神奈川県', '鳥取県']
...
# レポートを保存
report_file = r"c:\Users\user\SharedWorkspace\projects\NDB_Research_Hub\projects\NDB_XXX_slope_fracture\03_Analysis\data\interim\prefecture_coverage_report_final_v2.txt"
...
# メッシュ5235の確認
mesh_5235 = result_df[result_df['mesh_code'] == 5235]
if len(mesh_5235) > 0:
    print(f"\n=== メッシュ5235の確認 ===")
    print(f"都道府県: {mesh_5235.iloc[0]['prefectures']}")
